...
TaskManager.defineTask<LocationTaskData>(BACKGROUND_LOCATION_TASK, ({ data, error }) => {
  if (error) {
  ...
  if (latest) {
    console.log('Background location update', latest.coords.latitude, latest.coords.longitude);
  }
});
